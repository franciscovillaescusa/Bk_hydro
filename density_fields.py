...
#root = '/simons/scratch/sgenel/OUTPUT_RUNS/CosmologicalBoxes/CosmoVariance/L200n640/'
#prefix_snap = 'TNG_CVset'
#prefix_out  = 'TNG_Paco'
#snapnums = np.array([50, 40]) #z=0 and z=1

root = '/simons/scratch/fvillaescusa/Bk_foreman/DM_sims/'
prefix_snap = 'DM'
prefix_out = 'DM_Paco'
snapnums = np.array([4, 2]) #z=0 and z=1
...
for i in xrange(130):
    ...
    for snapnum in snapnums:
        ...
        snapshot = '%s/%s_%d/snap_%03d.hdf5'%(root,prefix_snap,i,snapnum)
        ...
        f_out = '%s_%s_%d_z=%.1f.hdf5'%(prefix_out,suffix,i,round(redshift))
        ...
        for j in xrange(filenum):
            ...
            # find the snapshot name
            if filenum>1:
                snapshot = '%s/%s_%d/snapdir_%03d/snap_%03d.%d.hdf5'\
                           %(root,prefix_snap,i,snapnum,snapnum,j)
            else:
                snapshot = '%s/%s_%d/snap_%03d.hdf5'%(root,prefix_snap,i,snapnum)
