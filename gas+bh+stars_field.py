...
runs = [#'/n/hernquistfs3/IllustrisTNG/Runs/L75n455TNG',
...
fouts = [#'TNG100-3', 'TNG300-3',
         #'TNG100-2', 'TNG300-2',
         'TNG100-1_gas+bh+stars', 
         'TNG300-1_gas+bh+stars'
        ]
...
for fout,run in zip(fouts,runs):
    ...
    for snapnum in snapnums:
        ...
        for i in xrange(filenum):
            ...
            print '%03d -----> Omega_b = %.4f  : %6.0f s'\
                %(i, M_total/(BoxSize**3*rho_crit), time.time()-start)

        f = h5py.File(f_out,'w')
        f.create_dataset('delta_gbhs',  data=delta_m)
        f.close()
